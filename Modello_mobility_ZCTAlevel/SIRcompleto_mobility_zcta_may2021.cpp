//modello SIR mobility con cityseed random ad ogni simulazione e indice di rischio a livello di zcta
...
int stampaInfetti = 0;
double wij,TRout,TRin,s, p, c, d,z;
int cityseed;
int foldernumber = 1; 		//città seed di partenza Ikk (cityseed = 1 -> (I)11 seed)

double R0; //varying during the weeks of simulations
double mu = 1/3.0;	// rate di guarigione [day^-1]           R0=beta/mu>1 per avere outbreak
map<int,double> beta;	// trasmissibilità (probabilità di trasmissione dell'infezione)
map<int,double> risk; //vettore di indice di rischio a livello di zcta
int infettiIniz = 10;
...
int main (int argc, char * argv[]){
	...
	ifstream popFILE ("./tot_pop_zcta.txt");
	if ( popFILE.is_open() ){
	...
	ifstream networkFILE ("./commuting_w1.txt");
	...
	//open file of risk index at zcta level
	ifstream rFILE ("./risk_index.txt");
	if ( rFILE.is_open() ){
		cout<<"Open risk index file \t"<<endl;
		while ( rFILE.good() ){

			while( rFILE >> i >> z ){//i node, j value
				risk[i] = z;
				cout << z<< " ";
			}
			rFILE.close();
	...
	}else cout << "\nUnable to open the risk index file"<< endl;
	...
	for(int l = 0; l < nrun; l++){  /////////// FOR SUI RUN, l will indicate the number of simulationS
		...
		//ad ogni simulazione vado ad inizializzare  le matrici in base al file di commuting e random cityseed
		//random cityseed
		srand(time(NULL));
		cityseed = rand()%177;
		cout << "\n cityseed:"<< cityseed << endl;
		...
		//inizializzo valori di R0 e beta ad ogni simulazione e poi vado ad aggiornarlo
		R0 = 3;
		fileOUTPUTmatrices << "\n Beta updated with risk_index: " << endl;
		for(int i=0; i< risk.size(); i++){
			beta[i] = R0*mu;
			beta[i] = beta[i]*risk[i];
			fileOUTPUTmatrices << beta[i] << " ";
		}

		cout << "\n R0= " << R0 << endl;

		fileOUTPUTmatrices<<"\n Cityseed iniziale:" << cityseed <<endl;
		...
		while(arrayIr != arrayNULL){
			...
			if(t==7){
				...
				cout << "\n Variazione commuting w2: \t" << endl;

				ifstream networkFILE ("./commuting_var_w2.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				//aggiorno diag dato il nuovo trafficOUt (commuters offdiagonal)
				for(int i = 0; i < Npop.size(); i++){
					d=0;
					for (int j = 0; j < Npop.size(); j++){
						if(i!=j){
							d = d + Stemp_matrix[i][j]; //il traffic out attuale
						}
					}
					diag[i] = Npop[i] - d;
				}

				//fileOUTPUTmatrices << "\n Diagonale matrice suscettibili: N_i - trafficout_i \t" << endl;
				//for(int i=0; i< Npop.size(); i++){
					//fileOUTPUTmatrices << diag[i] << " ";
				//}fileOUTPUTmatrices << endl;


				//fileOUTPUTmatrices << "\n Suscettibili at home per boro: \t" << endl; //numero di suscettibili che rientra sulla diag
				//for ( int i = 0; i < Npop.size(); i++){
			...
			}else if (t==14){ //end dell'IF di aggiornamento della prima settimana
				//apro il file di variazione relativa del commuting alla seconda settimana
				cout << "\n Variazione commuting w3: \t" <<endl;

				ifstream networkFILE (".commuting_var_w3.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				//when the variation is slighter > 1, we're assuming things remains the same
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w3: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				//fileOUTPUTmatrices << "\n Suscettibili at home per boro: \t" << endl;
				//for ( int i = 0; i < Npop.size(); i++){
					//fileOUTPUTmatrices << susceptiblehome[i] << endl;
				//} fileOUTPUTmatrices << endl;
			...
			}else if (t==21){ //end dell'IF di aggiornamento della SECONDA settimana
				//apro il file di variazione relativa del commuting alla TERZA settimana
				cout << "\n Variazione relativa w4:\t" << endl;

				ifstream networkFILE ("./commuting_var_w4.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				//when the variation is slighter > 1, we're assuming things remains the same
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w4: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
			...
			}else if (t==28){ //end dell'IF di aggiornamento della TERZA settimana
				//apro il file di variazione relativa del commuting alla QUARTA settimana
				cout << "\n Commuting variation w5: \t" << endl;

				ifstream networkFILE ("./commuting_w5.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				//when the variation is slighter > 1, we're assuming things remains the same
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w5: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				cout << "\n Commuting variation w6: \t" << endl;

				ifstream networkFILE ("./commuting_w6.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w6: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				fileOUTPUTmatrices << "\n updated R0 (less then 1)= " << R0 << endl;



			}else if (t==42){

				cout << "\n Commuting variation w7: \t" << endl;

				ifstream networkFILE ("./commuting_w7.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w7: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				cout << "\n Commuting variation w8: \t" << endl;

				ifstream networkFILE (".commuting_w8.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w8: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				cout << "\n Commuting variation w9: \t" << endl;


				ifstream networkFILE ("./commuting_w9.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w9: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
				...
				cout << "\n Commuting variation w10: \t" << endl;


				ifstream networkFILE ("./commuting_w10.txt");
				if ( networkFILE.is_open() ){ //open network file of initial commuting
				...
				fileOUTPUTmatrices<<"\n Variazione relativa commuting w10: \t"<< endl;
				for( int i = 0; i < Npop.size(); i++ ){
					for( int j = 0; j < Npop.size(); j++){
						fileOUTPUTmatrices << rel_var_matrix[i][j] << " " ;
					}fileOUTPUTmatrices << endl;
			...
			for(int j = 0; j < Npop.size(); j++){
				...
				for(int i = 0; i < Npop.size(); i++){
					...
					deltaS[i][j] = gsl_ran_binomial(r,beta[j]*forceW[j],arraySr[i][j]);//is ok to use arraySr of the last time step
		...
		/******************************************************* HOME TIME ***************************************************************/
			...
			for(int i = 0; i < Npop.size(); i++){
				...
				for(int j = 0; j < Npop.size(); j++){
					...
					deltaS[i][j] = gsl_ran_binomial(r,beta[j]*forceH[i],arraySr[i][j]);
					deltaR[i][j] = gsl_ran_binomial(r,0.5*mu,arrayIr[i][j]);
					deltaI[i][j] = deltaS[i][j] - deltaR[i][j];
			...
			///////////OUTPUT fileOUTPUTincidence means delta dei suscettibili da confrontare con dati reali
